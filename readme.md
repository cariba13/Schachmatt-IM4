...
2. Wird eine Figur auf das Feld gestellt, verändert sich die Lichtintensität.
...
Wenn die ausgelesenen Werte die gegebene Bedingung, 15 ≤ x ≤ 19, erfüllt, dann gilt das Rätsel als gelöst. Wir haben die Werte absichtlich relativ breit definiert, so dass nicht genau eine Stellung zur Lösung des Rätsels führt, sondern mehrere welche etwa gleich sind wie die perfekte Lösung. Dies bietet Raum für kleine Ungenauigkeiten oder Interpretationsspielraum in der Rätsellösung. Der Drehregler hat nur in Teilen zuverlässig funktioniert. Wenn man ihn zu schnell dreht, registriert der Sensor nicht alle einzelnen Bewegungen der Drehung. Daher mussten wir noch die Anweisung hinzufügen, den Regler langsam zu drehen. Zu dem aber später mehr. Aufgrund der schlechten Steckverbindungen mussten wir die Kabel direkt am Drehregler anlöten. Dies führte zu einer stabileren Verbindung, schränkte jedoch die Modularität ein.
...
3. loop() erstellen und Sensorwerte verarbeiten: 
    Rotary Encoder: attachInterrupt() aktivieren um Drehbewegung zu erkennen
                    Zahlenwerte als absolute Positionen definieren 
                    Delay einbauen um finalen Wert erst nach 5 Sekunden Stillstand an JSON weiterzugeben
    NFC Reader: Jede Sekunde eindeutige ID auslesen und an JSON weitergeben
    Distanzsensor: Alle 0,5 Sekunden Wert lesen und an JSON weitergeben
    Lichtsensor: Wert 0 oder 1 auslesen und an JSON weitergeben
...
Die Hauptseite mit vier Rätseln.
 ...
 Sind alle Rätsel gelöst erscheint ein vierstelliger Code.
...
Dieses Skript greift nur auf die Unterseiten der einzelnen Rätsel zu.
    ...
    Ruft regelmässig status_check.php auf.
    Vergleicht die Sensorwerte mit den erwarteten Bedingungen.
    Löst ein PopUp auf der Rätselseite aus, sobald eine richtige Eingabe getätigt wurde.
